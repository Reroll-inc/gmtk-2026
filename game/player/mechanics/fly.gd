...
var _flap_sound: AudioStream = preload("res://assets/sfx/flap.ogg")


func can_activate() -> bool:
	return _active && Input.is_action_pressed("fly")
...
func on_physics_process(delta: float) -> bool:
	...
	_player.velocity = dir * _player.PROPERTIES.fly_speed
...
func _play_flap(delta: float, inp: Vector2) -> void:
	...
	if _flap_timer > 0.0 or _player.is_on_floor():
		return

	_flap_timer = _player.PROPERTIES.fly_flap_interval

	_player.audio_player.stream = _flap_sound
	_player.audio_player.pitch_scale = randf_range(0.5, 1.5)
